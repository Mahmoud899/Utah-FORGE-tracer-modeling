"""
NumPy-backed simulation wrapper functions.

Drop-in replacements for the functions in simulation_options.py, using
RELAP_Numpy classes and dehoog_invert instead of mpmath.  The public API
(argument names, order, defaults) is identical to simulation_options.py so
callers only need to change the import line.

Performance note: eliminates the mpm.mp.dps setup overhead and replaces the
per-time-point invertlaplace loop with a single batched dehoog_invert call,
giving ~50-100x speedup for typical parameter ranges.
"""

import numpy as np

from .RELAP_Numpy import (
    GroundWaterFiniteMatrixSolution,
    GroundWaterFiniteMatrixSolution_2,
    GroundWaterInfiniteMatrixSolution,
    Input_Pulses_of_Tracer,
    Recirculation,
    WellboreStorage,
    PipelineDelay,
    RELAP_Modifed,
    Simulate_RELAP_Relative,
)
